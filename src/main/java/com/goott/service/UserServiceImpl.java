...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public Map<String, Object> changeProfileImg(MultipartFile file, String member_id) { //회원 이미지 저장
    ...
    public String getUserImgUrl(String member_id) {
        ...
        return userMapper.selectUserImg(member_id); // 프로필 이미지 주소
    }
    ...
    public String changeUserPw(String member_id, String member_pw) {
        ...
        // 비밀번호 변경 되었으면 비밀번호 초기화 상태 n으로 변경(비밀번호 초기화 신청 상태 고려)
        if (resultCount == 1) {
            //비밀번호 초기화 완료 업데이트
    ...
    public String forgotPassword(String member_id, String member_email) {
        ...
        int resultCount = userMapper.updatePw(member_id, member_pw); // 회원 비밀번호 초기화 비밀번호로 변경
        //비밀번호 초기화 컬럼 업데이트
    ...
    @Transactional
    @Override
    public String reviewHelpful(String member_id, int product_review_id) { //리뷰 좋아요 누르기
        //좋아요 눌렀는지 확인
        Map<String, Object> map = new HashMap<>();
        map.put("member_id", member_id);
    ...
    @Override
    public List<BasketVO> selectBasketList(String memberId) {
        return userMapper.selectBasketList(memberId);
    }
